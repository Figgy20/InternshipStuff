    ...
    public static void main(String[]args) throws IOException, InterruptedException{
        ...
        if (bpiCC.isNumberEqualTo16(creditCardNumber)) {
            if (bpiCC.isCreditCardNumberValid(creditCardNumber)) {
                System.out.println("Gross Purchase Amount: Php"+ amountPurchased);
                
                double vatAmount = bpiCC.computeVat(amountPurchased);
                System.out.println("VAT\t\t: Php:"+(vatAmount-amountPurchased));
                System.out.println("Total: "+(vatAmount+amountPurchased));
